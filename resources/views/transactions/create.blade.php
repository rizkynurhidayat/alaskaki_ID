...
@section('title', 'Tambah Transaksi - PT/CV')

@section('header_title', 'Tambah Transaksi')
@section('header_subtitle', 'Catat transaksi pemasukan atau pengeluaran baru')

@section('content')
<div class="max-w-lg mx-auto bg-white p-5 sm:p-8 rounded-2xl shadow-sm border border-slate-100">
    <h3 class="text-xl font-bold text-slate-800 mb-6">Tambah Transaksi Baru</h3>
    ...
    <form action="{{ route('transactions.store') }}" method="POST" class="space-y-5">
        ...
        <div id="product-select-container">
            ...
            <select id="product_id" name="product_id" class="w-full border border-slate-200 rounded-xl p-3 focus:ring-2 focus:ring-blue-100 focus:border-blue-500 outline-none transition">
                ...
                @foreach($products as $product)
                    <option value="{{ $product->id }}" data-price="{{ (int)$product->price }}" data-name="{{ $product->name }}" {{ old('product_id') == $product->id ? 'selected' : '' }}>
        ...
        <div>
            ...
            <input type="number" name="amount" value="{{ old('amount') }}" class="w-full border border-slate-200 rounded-xl p-3 focus:ring-2 focus:ring-blue-100 focus:border-blue-500 outline-none transition" placeholder="Contoh: 150000" min="1" required>
            @error('amount')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            ...
            <input type="date" name="transaction_date" value="{{ old('transaction_date', date('Y-m-d')) }}" class="w-full border border-slate-200 rounded-xl p-3 focus:ring-2 focus:ring-blue-100 focus:border-blue-500 outline-none transition" required>
        ...
        <div>
            ...
            <textarea name="description" rows="3" class="w-full border border-slate-200 rounded-xl p-3 focus:ring-2 focus:ring-blue-100 focus:border-blue-500 outline-none transition" placeholder="Contoh: Pembelian sabun pembersih" required>{{ old('description') }}</textarea>
            @error('description')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
            @enderror
        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Run initially to set correct state based on old values
        handleTypeChange();
    });
</script>
@endpush
